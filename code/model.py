...
def get_model(strategy):
    ...
    with strategy.scope():
        ...
        mel_spec = get_melspectrogram_layer(input_shape=None, n_fft=config.n_fft, win_length=config.win_length, hop_length=config.hop_length, window_name='hann_window', pad_begin=False, 
                                                pad_end=False, sample_rate=config.sample_rate, n_mels=config.n_mels, mel_f_min=0.0, mel_f_max=None, mel_htk=False, mel_norm='slaney', 
                                                return_decibel=True, db_amin=1e-05, db_ref_value=1.0, db_dynamic_range=80.0,
                                                input_data_format='default', output_data_format='default', name='melspectrogram')
        model = Sequential()
        model.add(mel_spec)
